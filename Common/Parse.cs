...
namespace Common
{
    ...
    public static class Parse
    {
        ...
        /// <summary>使用 Guid.TryParse 方法来尝试转为 Guid? 型，若失败，则返回 null 值</summary>
        /// <param name="obj">待转换的参数</param>
        /// <returns></returns>
        public static Guid? TryGuidAble(object obj)
        {
            if (obj == null) return null;
            Guid i = Guid.NewGuid();
            bool rs = Guid.TryParse(ToString(obj), out i);
            return rs ? new Guid?(i) : null;
        }


        #region string
        /// <summary>转为字符串，若 obj 为 null，则返回 "" 已调用Trim()方法</summary>
        ...
        /// <summary>转为字符串，若 obj 为 null，则返回 null 已调用Trim()方法</summary>
        /// <param name="obj"></param>
        /// <returns></returns>
        ...
        /// <summary>转为字符串，若 obj 为 null或空，则返回默认值 已调用Trim()方法</summary>
        /// <param name="obj"></param>
        ...
        /// <summary>转为guid，若 obj 为 null，则返回 null</summary>
        /// <param name="obj">相关类实例</param>
        /// <returns></returns>
        public static Guid? ToGuidAble(object obj)
        {
            if (obj == null) return null;

            Regex regex = new Regex(@"^[A-Fa-f0-9]{8}(-[A-Fa-f0-9]{4}){3}-[A-Fa-f0-9]{12}$");

            if (regex.IsMatch(obj.ToString())) return Guid.Parse(obj.ToString());
            return null;
        }
        #endregion
        ...
        public static string ToInt(object obj, out int result)
        {
            ...
            else
            {
                try
                {
                    result = Convert.ToInt32(ToDouble(obj));
                    return "";
                }
                catch (Exception e)
                {
        ...
        public static DateTime? TryDateTimeAbleFromMilliTimeStamp(object obj)
        {
            if (obj == null) return null;
            long milliTime = 0;
            if (!long.TryParse(obj.ToString(), out milliTime)) return null;
